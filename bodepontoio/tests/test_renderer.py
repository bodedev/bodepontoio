...
class TestSuccessJSONRenderer:
    ...
    def test_paginated_response(self):
        ...
        assert "_paginated" not in result["data"]

    def test_non_paginated_dict_not_treated_as_paginated(self):
        data = {"count": 42, "next": "http://example.com?page=2", "results": []}
        result, _ = render(data)
        assert result["success"] is True
...
class TestRendererIntegration:
    def test_login_wire_format(self, api_client, create_user):
        ...
        response = api_client.post(
            "/auth/login/",
            {"login": "render@example.com", "password": "testpassword123"},
        )
        body = response.json()
