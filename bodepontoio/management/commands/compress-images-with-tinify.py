...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        if not getattr(settings, 'TINYPNG_KEY', None):
            self.stderr.write(self.style.ERROR('TINYPNG_KEY não está configurado em settings.'))
            return
                    ...
                    for name in files:
                        if name.endswith(('.png', '.jpg', '.jpeg')):
                            ...
                            if OptimizedImageWithTinyPNG.objects.filter(path=img_path).exists():
                                ...
                                continue
                            img_size = os.path.getsize(os.path.join(root, name))
                            ...
                            if img_size > 250000:
                                ...
                                self.stdout.write(self.style.SUCCESS(f'Terminou otimização imagem: {img_path} com novo tamanho de {new_img_size}'))
                                OptimizedImageWithTinyPNG.objects.create(path=img_path)

            except FileNotFoundError:  # Não tem o folder especificado
                self.stderr.write(self.style.ERROR('Não encontrou Folder/Arquivo'))
